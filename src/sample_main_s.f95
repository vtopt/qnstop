! This file (sample_main_s.f95) contains a sample main program that calls
! QNSTOPS to minimize the Griewank function, and verify the installation.
...
! Use of QNSTOPS for deterministic global optimization, MODE = 'G'.
EVALlim = 0
ITERlim = 100
XI = 1.0_R8
CALL QNSTOPS(50, LB, UB, Obj_GR, XI, 'G', FMIN, STATUS, SWITCH=3,  &
...
! Use of QNSTOPS for (local) stochastic optimization, MODE = 'S'.
EVALlim = 0
ITERlim = 500
LB(1:2) = -1.0_R8 ! Lower bounds.
UB(1:2) = 7.0_R8  ! Upper bounds.
XI(1:2) = (/ 1.5_R8, 2.0_R8 /)
CALL QNSTOPS(2, LB(1:2), UB(1:2), Obj_GR, XI(1:2), 'S', FMIN, STATUS, &
  SWITCH=1, N=75, MAX_ITER=ITERlim, MAX_EVAL=EVALlim, TAU=10.0_R8, &
  GAMMAW=2.0_R8, ETA=0.1_R8)
IF ((STATUS .GE. 10) .OR. (FMIN > 1.0E-2_R8)) THEN
